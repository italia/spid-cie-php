...
class Message
{
    ...
    private static function processAssertion(
        ...
        if (!self::checkSign($idpMetadata, $assertion)) {
            ...
            */
            throw new SSP_Error\Exception('The assertion was not signed.');
        ...
        if($nameId == null || $nameId->getValue() == null || $nameId->getValue() == "") {
            throw new SSP_Error\Exception(
                'NameID value was not valid.'
            );
        }

        if($nameId->getFormat() == null || $nameId->getFormat() != "urn:oasis:names:tc:SAML:2.0:nameid-format:transient") {
            throw new SSP_Error\Exception(
        ...
        */
        ...
        $subjectConfirmationData = $subjectConfirmation[0]->getSubjectConfirmationData();
        ...
        $subjectConfirmationDataRecipient = $subjectConfirmationData->getRecipient();
        ...
        $subjectConfirmationDataInResponseTo = $subjectConfirmation[0]->getSubjectConfirmationData()->getInResponseTo();
        ...
        $subjectConfirmationDataNotOnOrAfter = $subjectConfirmation[0]->getSubjectConfirmationData()->getNotOnOrAfter();
        ...
        if($responseIssuer == null || $responseIssuer != $idpMetadata->getString('entityid')) {
            throw new SSP_Error\Exception(
        ...
        if($assertionIssuer == null || $assertionIssuer != $idpMetadata->getString('entityid')) {
            throw new SSP_Error\Exception(
