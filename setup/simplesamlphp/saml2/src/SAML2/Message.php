...
abstract class Message implements SignedElement
{
    ...
    protected function __construct($tagName, \DOMElement $xml = null)
    {
        ...
        /* SPID CUSTOM : ID attribute cannot be blank */
        if (!$xml->hasAttribute('ID') || trim($xml->getAttribute('ID'))=="") {
        ...
        /* SPID CUSTOM : check InResponseTo */
        if (!$xml->hasAttribute('InResponseTo') || trim($xml->getAttribute('InResponseTo'))=="") {
            throw new \Exception('Missing InResponseTo attribute on SAML message.');
        }

        $this->issueInstant = Utils::xsDateTimeToTimestamp($xml->getAttribute('IssueInstant'));

        /* SPID CUSTOM : Destination attribute cannot be blank */
        if (!$xml->hasAttribute('Destination') || trim($xml->getAttribute('Destination'))=="") {
            throw new \Exception('Missing Destination attribute on SAML message.');
        }
